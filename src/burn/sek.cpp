...
extern "C" int log_memory;
#define LOG_READ(v) if (log_memory) { QuarkLog("  " __FUNCTION__ " 0x%08x 0x%08x : %08x.\n", a, pr, ((unsigned int)pr > SEK_MAXHANDLER) ? (v) : -1); }
#define LOG_WRITE() if (log_memory) { QuarkLog("  " __FUNCTION__ " 0x%08x 0x%08x -> %08x.\n", a, pr, d); }
#else
#define LOG_READ(v)
#define LOG_WRITE()
#endif
...
//	bprintf(PRINT_NORMAL, _T("read16 0x%08X\n"), a);
	...
	pr = FIND_R(a);
   LOG_READ(*((unsigned short*)(pr + (a & SEK_PAGEM))));
	if ((unsigned int)pr >= SEK_MAXHANDLER) {
...
//	bprintf(PRINT_NORMAL, _T("read32 0x%08X\n"), a);
	...
	pr = FIND_R(a);
   LOG_READ(*((unsigned int*)(pr + (a & SEK_PAGEM))));
...
//	bprintf(PRINT_NORMAL, _T("  - irq line %i -> %i\n"), line, status);
	...
	if (status) {
		...
		if (nSekCPUType[nSekActive] == 0) {
         ...
         QUARK_LOG_VAR(nSekCyclesTotal);
         QUARK_LOG_VAR(nSekCyclesDone);
         QUARK_LOG_VAR(nSekCyclesSegment);
         QUARK_LOG_VAR(m68k_ICount);
   } else {
#endif
...
#ifdef EMU_A68K
   QUARK_LOG_STR("Using EMU_A68K");
	if (nSekCPUType[nSekActive] == 0) {
		nSekCyclesDone = 0;
		nSekCyclesSegment = nCycles;
		do {
			m68k_ICount = nSekCyclesToDo = nSekCyclesSegment - nSekCyclesDone;
         QUARK_LOG_STR("Updating m68k_ICount ");
         QUARK_LOG_VAR(nSekCyclesSegment);
         QUARK_LOG_VAR(nSekCyclesDone);
         QUARK_LOG_VAR(m68k_ICount);
...
#ifdef EMU_M68K
      QUARK_LOG_STR("Using EMU_M68K");

		nSekCyclesToDo = nCycles;

      QUARK_LOG_VAR2("Pre m68k_execute", nCycles);
		nSekCyclesSegment = m68k_execute(nCycles);
      QUARK_LOG_VAR2("Post m68k_execute", nSekCyclesSegment);
...
#endif
   QUARK_LOG_STR("SekRun returning");
   QUARK_LOG_VAR(nSekCyclesToDo);
   QUARK_LOG_VAR(nSekCyclesTotal);
   QUARK_LOG_VAR(nSekCyclesDone);
   QUARK_LOG_VAR(nSekCyclesSegment);
...
int SekScan(int nAction)
{
   ...
   for (int i = 0; i <= nSekCount; i++) {
      ...
      if (nSekCPUType[i] == 0) {
          ...
          * Experimental
         char name[1024];
         sprintf(name, "SekExt #%d", i);
         ba.Data = SekExt[i];
         ba.nLen = sizeof(SekExt[i]);
         ba.szName = name;
         BurnAcb(&ba);
         */
#endif
