...
int RunFrame(int bDraw, int bPause)
{
	...
	if (bPause) {
		...
		if (NetworkGame()) {
		   ...
		   if (NetworkGetInput()) {			// Synchronize input with Kaillera
			   return 0;
		   }
		} else {
			if (nReplayStatus == 2) {
...
int RunMessageLoop()
{
	...
	do {
		...
		while (1) {
         if (nVidEmbeddedWindow && !IsWindow((HWND)nVidEmbeddedWindow)) {
            break;
         }
						...
						switch (Msg.wParam) {
							...
							case VK_RETURN: {
								if (hwndChat) {
									...
									if (i) {
                              char *text = TCHARToANSI(EditText, NULL, 0);
                              if (QuarkNetworkGame()) {
							...
							case VK_BACK: {
                        /*
                         * Ignore the backspace key if it was pressed while the chat window was up
                         */
                        if (!hwndChat) {
								   bShowFPS = !bShowFPS;
								   if (bShowFPS) {
