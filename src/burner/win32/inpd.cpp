...
static int InpdUseUpdate()
{
	...
	for (i = 0, pgi = GameInputs + nGameInpCount; i < nMacroCount; i++, pgi++) {
		LVITEM LvItem;
		TCHAR* pszVal = NULL;
...
int InpdUpdate()
{
	...
	// Update the values of all the inputs
	for (i = 0, pgi = GameInputs, plv = LastVal; i < nGameInpCount; i++, pgi++, plv++) {
		LVITEM LvItem;
		TCHAR szVal[16];
...
static int ListItemActivate()
{
	...
	if (bii.nType & BIT_GROUP_CONSTANT) {
		...
		if (GameInputs[nSel].nInput == GIT_MACRO_CUSTOM) {
#if 0
			InpMacroCreate(nSel);
#endif
		} else {
...
int UsePreset(bool bMakeDefault)
{
	...
	if (nPci <= 7) {
		...
		for (unsigned int i = 0, j = 0; i < nGameInpCount; i++) {
			if (GameInputs[i].Input.pVal == NULL) {
				continue;
			}

			if (GameInputs[i].nInput == 0) {
				DeleteInput(j);
			}
			...
			switch (plvcd->nmcd.dwDrawStage) {
				...
				case CDDS_ITEMPREPAINT:
					if (plvcd->nmcd.dwItemSpec < nGameInpCount) {
						if (GameInputs[plvcd->nmcd.dwItemSpec].nType & BIT_GROUP_CONSTANT) {

							if (GameInputs[plvcd->nmcd.dwItemSpec].nInput == 0) {
							...
							if (GameInputs[plvcd->nmcd.dwItemSpec].nType == BIT_DIPSWITCH) {
					...
					if (plvcd->nmcd.dwItemSpec >= nGameInpCount) {
						if (GameInputs[plvcd->nmcd.dwItemSpec].Macro.nMode) {
