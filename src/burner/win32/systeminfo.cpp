...
#define CODE(x) { EXCEPTION_ ## x, #x }
               CODE(ACCESS_VIOLATION),
               CODE(ARRAY_BOUNDS_EXCEEDED),
               CODE(BREAKPOINT),
               CODE(DATATYPE_MISALIGNMENT),
               CODE(FLT_DENORMAL_OPERAND),
               CODE(FLT_DIVIDE_BY_ZERO),
               CODE(FLT_INEXACT_RESULT),
               CODE(FLT_INVALID_OPERATION),
               CODE(FLT_OVERFLOW),
               CODE(FLT_STACK_CHECK),
               CODE(FLT_UNDERFLOW),
               CODE(ILLEGAL_INSTRUCTION),
               CODE(IN_PAGE_ERROR),
               CODE(INT_DIVIDE_BY_ZERO),
               CODE(INT_OVERFLOW),
               CODE(INVALID_DISPOSITION),
               CODE(NONCONTINUABLE_EXCEPTION),
               CODE(PRIV_INSTRUCTION),
               CODE(SINGLE_STEP),
               CODE(STACK_OVERFLOW),
#undef CODE
            };
				...
				hCodeFont = CreateFont(18, 0, 0, 0, FW_BOLD, 0, 0, 0, 0, 0, 0, ANTIALIASED_QUALITY, FF_MODERN, _T("Lucida Console"));
				SendDlgItemMessage(hDlg, IDC_SYSINFO_CODE, WM_SETFONT, (WPARAM)hCodeFont, (LPARAM)0);

            char *szErrorCode = "Unknown Exception";
            for (int i = 0; i < sizeof(codes) / sizeof(codes[0]); i++) {
               if (pExceptionPointers->ExceptionRecord->ExceptionCode == codes[i].ExceptionCode) {
                  szErrorCode = codes[i].Text;
                  break;
               }
            }
				_stprintf(szText, _T("%s #%08X.%08X"), szErrorCode, pExceptionPointers->ExceptionRecord->ExceptionCode, (unsigned int)pExceptionPointers->ExceptionRecord->ExceptionAddress);
