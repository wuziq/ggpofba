...
struct GameInp* GameInputs = NULL;
...
void GameInpCheckLeftAlt()
{
	...
	for (i = 0, pgi = GameInputs; i < (nGameInpCount + nMacroCount); i++, pgi++) {
...
int GameInpBlank(int bDipSwitch)
{
	...
	for (i = 0, pgi = GameInputs; i < nGameInpCount; i++, pgi++) {
...
{
	struct GameInp* pgi = GameInputs + nGameInpCount;
...
static int AddCustomMacro(TCHAR* szValue, bool bOverWrite)
{
	...
	for (unsigned int j = nGameInpCount; j < nGameInpCount + nMacroCount; j++) {
		if (GameInputs[j].nInput == GIT_MACRO_CUSTOM) {
			if (LabelCheck(szQuote, ANSIToTCHAR(GameInputs[j].Macro.szName, NULL, 0))) {
				nInput = j;
				break;
			}
		}
	...
	_tcscpy(szQuote, ANSIToTCHAR(GameInputs[nInput].Macro.szName, NULL, 0));
	...
	if ((szValue = LabelCheck(szEnd, _T("undefined"))) != NULL) {
		...
		if ((szValue = LabelCheck(szEnd, _T("switch"))) != NULL) {
			...
			if (bOverWrite || GameInputs[nInput].Macro.nMode == 0) {
				GameInputs[nInput].Macro.Switch.nCode = (unsigned short)_tcstol(szValue, &szValue, 0);
			}
	...
	if (nMode >= 0) {
		...
		for (int i = 0; i < 4; i++) {
			GameInputs[nInput].Macro.pVal[i] = NULL;
			GameInputs[nInput].Macro.nVal[i] = 0;
			GameInputs[nInput].Macro.nInput[i] = 0;
			...
			for (unsigned int j = 0; j < nGameInpCount; j++) {
				...
				if (szString && szEnd) {
					GameInputs[nInput].Macro.pVal[i] = bii.pVal;
					GameInputs[nInput].Macro.nInput[i] = j;

					GameInputs[nInput].Macro.nVal[i] = (unsigned char)_tcstol(szEnd, &szValue, 0);
		...
		if (nFound) {
			if (GameInputs[nInput].Macro.pVal[nFound - 1]) {
				GameInputs[nInput].nInput = GIT_MACRO_CUSTOM;
				GameInputs[nInput].Macro.nMode = nMode;
				if (bCreateNew) {
					nMacroCount++;
				}
				return 0;
	...
	while (_fgetts(szLine, sizeof(szLine), h)) {
		...
		if (nConfigMinVersion <= nFileVersion && nFileVersion <= nBurnVer) {
			...
			if (szValue) {
				...
				if (GameInputs[i].nInput == 0 || bOverWrite) {				// Undefined - assign mapping
					StringToInp(GameInputs + i, szEnd);
				}
			}
			...
			if (szValue) {
				...
				if (i != ~0U) {
					i += nGameInpCount;
					if (GameInputs[i].Macro.nMode == 0 || bOverWrite) {	// Undefined - assign mapping
						StringToMacro(GameInputs + i, szEnd);
					}
				}
			}
...
int GameInpRead(TCHAR* szVal, bool bOverWrite)
{
	...
	if (bOverWrite || GameInputs[i].nInput == 0) {
...
int GameInpMacroRead(TCHAR* szVal, bool bOverWrite)
{
	...
	if (i != ~0U) {
		i += nGameInpCount;
		if (GameInputs[i].Macro.nMode == 0 || bOverWrite) {
			StringToMacro(GameInputs + i, szEnd);
		}
	}
