...
extern char *m68ki_instruction_name_table[0x10000];
...
char *m68ki_instruction_name_table[0x10000] = { 0 };
...
void m68ki_build_opcode_table(void)
{
	...
	{
		/* default to illegal */
		m68ki_instruction_name_table[i] = 0;
	...
	while(ostruct->mask != 0xff00)
	{
		for(i = 0;i < 0x10000;i++)
		{
			...
			{
				m68ki_instruction_name_table[i] = ostruct->name;
	...
	while(ostruct->mask == 0xff00)
	{
		for(i = 0;i <= 0xff;i++)
		{
			m68ki_instruction_name_table[ostruct->match | i] = ostruct->name;
	...
	while(ostruct->mask == 0xf1f8)
	{
		for(i = 0;i < 8;i++)
		{
			for(j = 0;j < 8;j++)
			{
				...
				m68ki_instruction_name_table[instr] = ostruct->name;
	...
	while(ostruct->mask == 0xf1ff)
	{
		for(i = 0;i <= 0x07;i++)
		{
			m68ki_instruction_name_table[ostruct->match | (i << 9)] = ostruct->name;
	...
	{
		m68ki_instruction_name_table[ostruct->match] = ostruct->name;
